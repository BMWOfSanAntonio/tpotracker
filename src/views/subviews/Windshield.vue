<template>
  <v-container class="sublet mt-10">
    <!-- // * Title -->
    <div class="mb-4 display-2 font-weight-thin">Windshield</div>
    <!-- // * Table: Start -->
  ...
  methods: {
    ...
    completeRepair(item) {
      ...
      db.collection("tpo")
        .doc(filterrepair[0].id)
        .update({
          repairs: firebase.firestore.FieldValue.arrayRemove("Windshield")
        });
    }
  },
  // * Database
  firestore() {
    return {
      sublet: db
        ...
        .where("repairs", "array-contains", "Windshield")
