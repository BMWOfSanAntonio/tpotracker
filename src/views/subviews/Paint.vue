<template>
  <v-container class="sublet mt-10">
    <!-- // * Title -->
    <div class="mb-4 display-2 font-weight-thin">Paint</div>
    <!-- // * Table: Start -->
  ...
  methods: {
    ...
    completeRepair(item) {
      ...
      db.collection("tpo")
        ...
        .update({
          repairs: firebase.firestore.FieldValue.arrayUnion("Paint - Complete")
        });
      db.collection("tpo")
        .doc(filterrepair[0].id)
        .update({
          repairs: firebase.firestore.FieldValue.arrayRemove("Paint")
        });
    }
  },
  // * Database
  firestore() {
    return {
      sublet: db
        ...
        .where("repairs", "array-contains", "Paint")
