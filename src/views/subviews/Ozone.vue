<template>
  <v-container class="sublet mt-10">
    <!-- // * Title -->
    <div class="mb-4 display-2 font-weight-thin">Ozone</div>
    <!-- // * Table: Start -->
  ...
  methods: {
    ...
    completeRepair(item) {
      ...
      db.collection("tpo")
        ...
        .update({
          repairs: firebase.firestore.FieldValue.arrayUnion("Ozone - Complete")
        });
      db.collection("tpo")
        .doc(filterrepair[0].id)
        .update({
          repairs: firebase.firestore.FieldValue.arrayRemove("Ozone")
        });
    }
  },
  // * Database
  firestore() {
    return {
      sublet: db
        ...
        .where("repairs", "array-contains", "Ozone")
